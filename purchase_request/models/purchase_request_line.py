...
class PurchaseRequestLine(models.Model):
    """Inherited Model purchase request line."""
    ...
    qty_cancelled = fields.Float(
        'Qty Cancelled', digits='Product Unit of Measure',
    ...
    def _compute_is_editable(self):
        for rec in self:
            if rec.request_id.state in ('approved', 'rejected', 'done'):
    ...
    #                 rec.supplier_id = rec.product_id.seller_ids[0].id
    ...
    def do_cancel(self):
        """Action to perform when cancelling a purchase request line."""
        self.write({'cancelled': True})

    def do_uncancel(self):
        """Action to perform when uncancelling a purchase request line."""
        self.write({'cancelled': False})

    def write(self, vals):
        """Override wirte method."""
